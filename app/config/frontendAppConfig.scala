...
class FrontendAppConfig @Inject() (val config: Configuration, val environment: Environment) extends Logging {
  ...
  lazy val enablePlayHmrcAssessmentFeedbackView: Boolean =
    playHmrcViews && config.getOptional[Boolean]("enablePlayHmrcAssessmentFeedbackView").getOrElse(false)
  ...
  lazy val qualtricsBaseUrl: String = config.get[String]("qualtrics.base.url")
